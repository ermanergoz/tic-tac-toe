...
const Board = (props: BoardProps) => {
  ...
  const [data, setData] = useState(Array<string>(9).fill(""));
  ...
  const draw = (event: any, index: number) => {
    ...
    if (data[index - 1] === "" && props.winner === "") {
      // Draws X if it's player 1's turn else draws O
      const current = turn === 0 ? "E" : "O";
  ...
  }, [props.reset, props.setReset, props.setWinner]);
  ...
  useEffect(() => {
    ...
    if (checkWin()) {
      props.setWinner(turn === 0 ? "Player 2 Wins!" : "Player 1 Wins!");
    } else if (checkTie()) {
      // Setting the winner to tie in case of a tie
      props.setWinner("It's a Tie!");
    }
  });
